...
export interface AggregateDecisionData {
...
interface AggregateDecisionPageProps {
  category: string;
  secondOrderName: string;
  groupIndex: number;
  totalGroups: number;
  offenseCount: number;
  existingDecision?: AggregateDecisionData;
  onBack: () => void;
  onNext: (decision: AggregateDecisionData) => void;
}

export function AggregateDecisionPage({
  category,
  secondOrderName,
  groupIndex,
  totalGroups,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const response: AggregateDecisionData = {
      decision,
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-lg p-8">
          ...
          >
            ...
            <p className="text-sm opacity-90">
              Aggregate decision for {offenseCount} {offenseCount === 1 ? 'offense' : 'offenses'}
            </p>
          </div>
            ...
            <div className="mb-8 p-6 bg-purple-50 rounded-lg border-2 border-purple-200 space-y-4">
              <div>
                ...
                <p className="text-gray-600 text-sm mb-4">
                  Choose the job specific risks that this conviction may be related to (will apply to all {offenseCount} offenses in this group)
                </p>
              </div>
